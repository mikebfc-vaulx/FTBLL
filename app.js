...
function startSinglePlayer() {
  ...
  state.auctionPool = buildBalancedAuctionPool(players, state.config.rounds);
  state.playerIndex = 0;
  state.simulation = null;
  showView("game");
  beginAuction();
...
function applyMultiplayerSnapshot(snapshot) {
  ...
  state.config = {
    ...
    tactic: currentManager?.tactic || "balanced"
  };
...
function renderLiveTable(standings) {
  $("liveTable").classList.add("live-league-table");
  $("liveTable").innerHTML = `
    ...
    ${standings
      .map((manager, index) => {
        ...
        return `<div class="league-row ${isRealMultiplayer ? "real-player-row" : ""}" ${rowStyle}><span>${index + 1}</span><strong>${manager.name}</strong><span>${manager.stats.played}</span><span>${manager.stats.won}</span><span>${manager.stats.drawn}</span><span>${manager.stats.lost}</span><span>${manager.stats.gf}</span><span>${manager.stats.ga}</span><span>${gd > 0 ? `+${gd}` : gd}</span><strong>${manager.stats.points}</strong></div>`;
      })
      .join("")}
  `;
}
...
function renderSquadBuilder() {
  ...
  $("footballPitch").innerHTML = lineup.starters
    .map((slot) => {
      ...
      return `
        <div class="pitch-slot ${statusClass} ${player && state.selectedLineupPlayerId === player.uid ? "selected-player" : ""}" data-slot-id="${slot.id}" ${player ? `draggable="true" data-player-id="${player.uid}"` : ""} style="left:${slot.x}%; top:${slot.y}%;">
  ...
  $("benchList").innerHTML = lineup.bench.length
    ? lineup.bench
        .map(
          (player) => `
            <div class="bench-item ${state.selectedLineupPlayerId === player.uid ? "selected-player" : ""}" draggable="true" data-player-id="${player.uid}">
...
function playMatch(home, away) {
  ...
  return {
    ...
    awayGoals,
    shots: {
      home: 5 + Math.floor(Math.random() * 9),
      away: 5 + Math.floor(Math.random() * 9)
    },
    events: []
  };
}
      ...
      (round) => `
        <div class="calendar-round">
          ...
          <div class="calendar-match-grid">
            ${round.matches
              .map((match) => `<div class="calendar-match"><span>${match.home} <strong>${match.homeGoals}-${match.awayGoals}</strong> ${match.away}</span><small>Tiri ${match.shots.home}-${match.shots.away}${match.events?.length ? ` - ${match.events.slice(0, 2).join(" | ")}` : ""}</small></div>`)
              .join("")}
          </div>
        </div>
      `
