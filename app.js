...
async function createMultiplayerLobby() {
  try {
    ...
    const payload = {
      ...
      formation: $("multiFormationSelect").value,
      credits: clampNumber($("multiCreditsInput").value, 250, 1200),
      rounds: clampNumber($("multiAuctionPlayersInput").value, 12, players.length)
    };
...
function applyMultiplayerSnapshot(snapshot) {
  ...
  } else if (snapshot.status === "auction") {
...
function renderLobby(snapshot) {
  ...
  $("lobbyPlayersList").innerHTML = snapshot.managers
    .map((manager) => `<div class="rival-card"><div class="rival-card-head"><strong>${manager.name}</strong><span>${manager.id === snapshot.hostId ? "Host" : "Pronto"}</span></div></div>`)
    .join("");
  $("startLobbyAuctionBtn").disabled = !state.multiplayer.isHost || snapshot.managers.length < 2;
}
...
async function multiplayerReady() {
  await api(`/api/lobbies/${state.multiplayer.code}/ready`, { playerId: state.multiplayer.playerId, tactic: $("tacticSelect").value });
  pollMultiplayer();
}
...
function closeAuction() {
  ...
  if (state.leaderId) {
    ...
    winner.squad.push({ ...player, price: state.currentBid, generated: false, stats: createEmptyPlayerStats() });
...
function renderSquadBuilder() {
  ...
  $("lineupSummary").textContent = `${state.config.formation} - ${lineup.starters.length - missing}/11 titolari pronti. Giocatori generati: ${generated}.`;
  ...
  $("footballPitch").innerHTML = lineup.starters
    .map((slot) => {
      ...
      const overall = player ? player.overall : "--";
      const source = player && player.generated ? "Auto" : player ? "Asta" : "Serve giocatore";
      return `
        <div class="pitch-slot ${statusClass}" style="left:${slot.x}%; top:${slot.y}%;">
          <strong>${slot.role}</strong>
          <span>${name}</span>
          <small>${source} - OVR ${overall}</small>
        </div>
      `;
    })
    .join("");
  ...
  $("benchList").innerHTML = lineup.bench.length
    ? lineup.bench
        ...
        .join("")
    : `<p class="muted">Nessun giocatore in panchina. I profili fuori ruolo compariranno qui.</p>`;
}
...
function renderAuction() {
  ...
  $("roundLabel").textContent = state.mode === "multi" ? "Asta multiplayer" : `Asta ${state.playerIndex + 1}/${state.auctionPool.length}`;
  $("timerLabel").textContent = state.timeLeft.toFixed(1);
...
function showResults() {
  ...
  $("resultTitle").textContent = userPosition === 1 ? "Hai vinto il campionato" : `Hai chiuso ${userPosition} posto`;
