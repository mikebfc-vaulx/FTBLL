...
function fillVacancies(lobby, playerId) {
  const manager = lobby.managers.find((item) => item.id === playerId);
  if (!manager) return 0;
  const needed = [...formationNeeds[manager.formation || lobby.settings.formation]];
  ...
  for (let round = 0; round < 2; round += 1) {
    for (let i = 0; i < lobby.managers.length; i += 1) {
      for (let j = i + 1; j < lobby.managers.length; j += 1) {
        const match = playMatch(lobby, round === 0 ? lobby.managers[i] : lobby.managers[j], round === 0 ? lobby.managers[j] : lobby.managers[i]);
        rounds.push({ round: roundNumber, matches: [match], standings: tableSnapshot(lobby.managers) });
        roundNumber += 1;
      }
    }
  }
...
function addSimulationBots(lobby) {
  ...
  for (let i = 0; i < botCount; i += 1) {
    ...
    bot.isBot = true;
    bot.ready = true;
    bot.squad = formationNeeds[lobby.settings.formation].map((role) => makeBotPlayer(role));
    lobby.managers.push(bot);
  }
  lobby.botsAdded = true;
}

function makeBotPlayer(role) {
  const generated = makeGenerated(role);
  generated.overall = 70 + Math.floor(Math.random() * 16);
...
async function handleApi(req, res, parts, url) {
  ...
  if (req.method === "POST" && parts[3] === "settings") {
    ...
    lobby.pool = shuffle(players).slice(0, lobby.settings.rounds);
    lobby.managers.forEach((manager) => {
