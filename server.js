...
function buildBalancedAuctionPool(sourcePlayers, rounds) {
  const roleGroups = {
    keeper: ["POR"],
    defense: ["DC", "TS", "TD"],
    midfield: ["MED", "CC", "COC"],
  ...
  const pools = Object.fromEntries(
    Object.entries(roleGroups).map(([group, roles]) => [group, shuffle(sourcePlayers.filter((player) => roles.includes(player.role)))])
  );

  groupWeights.forEach(([group, weight], index) => {
    const desired = index === groupWeights.length - 1
      ? target - selected.length
      : Math.max(1, Math.round(target * weight));
    while (selected.length < target && pools[group]?.length && selected.filter((player) => roleGroups[group].includes(player.role)).length < desired) {
      const player = pools[group].pop();
...
function playMatch(lobby, home, away) {
  ...
  events.push(...randomMatchEvents(home, away));
  ...
  return {
    ...
    shots: {
      home: Math.max(homeGoals + 2, sampleShots(homeStrength)),
      away: Math.max(awayGoals + 2, sampleShots(awayStrength))
    }
  };
}
...
function teamStrength(lobby, manager) {
  ...
  const avg = starters.reduce((sum, player, index) => sum + effectiveOverall(player, roles[index]), 0) / starters.length;
  const tacticBonus = {
    balanced: 1,
    pressing: 1.018,
    counter: 1.016,
  ...
  return avg * (tacticBonus[manager.tactic] || 1);
}
...
function assignGoal(lobby, manager, teamName) {
  ...
  let text = `${minute}' Gol ${teamName}: ${scorer.name}`;
...
function randomMatchEvents(home, away) {
  const teams = [home, away];
  const events = [];
  teams.forEach((team) => {
    const starters = team.squad.slice(0, 11);
    if (Math.random() < 0.55 && starters.length) events.push(`${1 + Math.floor(Math.random() * 90)}' Giallo ${team.name}: ${starters[Math.floor(Math.random() * starters.length)].name}`);
    if (Math.random() < 0.12 && starters.length) events.push(`${1 + Math.floor(Math.random() * 90)}' Rosso ${team.name}: ${starters[Math.floor(Math.random() * starters.length)].name}`);
    if (Math.random() < 0.1 && starters.length) events.push(`${1 + Math.floor(Math.random() * 90)}' Rigore sbagliato ${team.name}: ${starters[Math.floor(Math.random() * starters.length)].name}`);
  });
  return events.sort(() => Math.random() - 0.5).slice(0, 4);
}

function sampleShots(strength) {
  return 5 + Math.floor(Math.random() * 8 + Math.max(0, strength - 70) / 5);
}
...
async function handleApi(req, res, parts, url) {
  ...
  if (req.method === "POST" && parts[1] === "lobbies" && parts.length === 2) {
    ...
    const lobby = {
      ...
      settings,
      managers: [host],
      pool: buildBalancedAuctionPool(players, settings.rounds),
  ...
  if (req.method === "POST" && parts[3] === "join") {
    ...
    const manager = newPlayer(body.name, lobby.settings.credits, lobby.settings.formation, false, nextColor(lobby));
  ...
  if (req.method === "POST" && parts[3] === "settings") {
    ...
    lobby.pool = buildBalancedAuctionPool(players, lobby.settings.rounds);
    lobby.managers.forEach((manager) => {
      manager.credits = lobby.settings.credits;
      manager.ready = false;
